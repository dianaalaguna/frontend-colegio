...
export class UsuariosComponent implements OnInit {
  usuarios: User[] = [];
  // Usar el tipo 'User' en lugar de 'Partial<User>'
  editCache: { [key: string]: User } = {}; // Cambiamos a tipo completo User
  ...
  loadUsuarios(): void {
    this.userService.getUsers().subscribe({
      next: (users) => {
        this.usuarios = users;
        // Inicializar el cache de edición
  ...
  saveChanges(username: string): void {
    ...
    const confirmed = window.confirm(`¿Estás seguro de que deseas modificar los datos del usuario: ${username}?`);
    if (!confirmed) return;
  ...
  deleteUser(username: string): void {
    const confirmed = window.confirm(`¿Estás seguro de que deseas eliminar al usuario: ${username}?`);
    if (!confirmed) return;
